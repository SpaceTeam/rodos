	...
	switch (type)
	{
		case POLL_CHANNEL:
			if (paramVal < channelConfigs[0]->configs->channelNumberStart || paramVal > channelConfigs[0]->configs->channelNumberEnd)
				...
				pollChannel = static_cast<uint16_t>(paramVal);
			break;
		case DATA_CHANNEL:
			if (paramVal < channelConfigs[0]->configs->channelNumberStart || paramVal > channelConfigs[0]->configs->channelNumberEnd)
				return false;
			else
				dataChannel = static_cast<uint16_t>(paramVal);
			break;
		case NUMBER_OF_SLAVES:
			if (paramVal < 0 || static_cast<uint32_t>(paramVal) > MAX_NUMBER_SLAVES )
				return false;
			else
				numberOfSlaves = static_cast<uint8_t>(paramVal);
			break;
...
{	
	...
	else  // If the previous packet was not completely received yet, concatenation is needed.
	{
		...
		rxReceived = static_cast<uint16_t>(rxReceived + RAIL_ReadRxFifo(railHandle, rxPtr + _rxReceived,static_cast<uint16_t>(_rxExpected - _rxReceived)));
	}
}
...
void HAL_RADIO_MASTER::initPoll()
{
	...
	// 12 bits allow for a maximum size of 4096 bytes, but the protocol limit is specified in the MAX_NETWORK_MESSAGE_LENGTH macro.
	// This specifications are stored in the *.isc file, generated by Simplicity Studio.
	pollPacket[0] = static_cast<uint8_t>((3+numberOfSlaves - PACKET_HEADER_LEN) >> 8);
	pollPacket[1] = static_cast<uint8_t>((3+numberOfSlaves - PACKET_HEADER_LEN) & 0xFF);
...
void HAL_RADIO_MASTER::initMessages(int packetLength)
{
	...
	// 12 bits allow for a maximum size of 4096 bytes, but the protocol limit is specified in the MAX_NETWORK_MESSAGE_LENGTH macro.
	// This specifications are stored in the *.isc file, generated by Simplicity Studio.
	header[0] = static_cast<uint8_t>((packetLength + 1) >> 8);
	header[1] = static_cast<uint8_t >((packetLength + 1) & 0xFF);
	...
	switch (type)
	{
		...
		case DATA_CHANNEL:
			if (paramVal < channelConfigs[0]->configs->channelNumberStart || paramVal > channelConfigs[0]->configs->channelNumberEnd)
				...
				dataChannel = static_cast<u_int16_t >(paramVal);
			break;
		case SLAVE_ID:
			if (paramVal < 0 || static_cast<uint32_t>(paramVal) > MAX_NUMBER_SLAVES )
				return false;
